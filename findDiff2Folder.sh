...
arr=($(find $dir1 -name "*"))
PUT 13 5; echo "Done! Total file = ${#arr[@]}"
...
for file in "${arr[@]}"; do
    ...
    if [[ -f "$file" ]]; then 
        ...
        if [[ -d "$path_of_dir2" ]]; then                               # If this folder in dir2
            if [[ ! `find "$path_of_dir2" -name "$base_name"` ]]; then  # file No appear in dir2
               ...
               # PUT 10 5; printf "Dir1 : Size = $size1 KB, ${GREEN}%-109s${NC}" "$file" 
               # PUT 11 5; printf "Dir2 : Size = $size2 KB, ${GREEN}%-109s${NC}" "$path_of_dir2$base_name" 
    ...
    PUT 15 5; printf "Time passed${BLUE} 0%1.4smin:%1.1ssec ${NC}" "$minute" "$sec"
done
